...
make_plots <- function(summary_df, subspace_df, stats_df, diag_df = NULL,
  ...
  # show_leg: only ONE panel per figure carries the guide. Legend keys are drawn from
  # each layer's geom params, so panels differing in point size or in a mapped
  # linetype produce NON-IDENTICAL guides that patchwork cannot merge -- the cause of
  # duplicated legends. Suppressing the guide everywhere else removes the matching
  # problem entirely; `limits = lv` ensures the surviving guide lists every method.
  line_metric <- function(d, yv, ytitle, title, hy = NA, ylim = c(0, 1.02), gap = 0.045,
                          lv = NULL, show_leg = TRUE) {
    ...
    if (!show_leg) g <- g + guides(color = "none")
    g
  }
  ...
  make_comp_fig <- function(side_lab, side_disp) {
    ...
    for (l in seq_len(r)) {
      ...
      p_t1 <- line_metric(dS %>% filter(status == "null"),   "rejection",
                          ...
                          show_leg = TRUE)                    # the ONLY guide in this figure
      p_pw <- line_metric(dS %>% filter(status == "signal"), "rejection",
                          "Power", "Power", 1.0, lv = lv, show_leg = FALSE)
      ...
      p_cov <- p_cov + coord_cartesian(ylim = c(0, 1.02)) + guides(color = "none")
      p_sap <- line_metric(dst, "size_adj_power", "Size-adjusted power",
                           "Size-adjusted power", NA, ylim = c(0, 1.02), lv = lv,
                           show_leg = FALSE)
      # FLAT wrap_plots, not (a|b)/(c|d): the latter nests two sub-patchworks and
      # guides="collect" then yields one legend PER nesting branch.
      figs[[l]] <- wrap_plots(p_t1, p_pw, p_cov, p_sap, ncol = 2) +
  ...
  mk_contrast <- function(l) {
    ...
    b2 <- ggplot(d, aes(factor(n), coverage, fill = estimator)) +
      ...
      coord_cartesian(ylim = c(0, 1.02)) + th + leg2 + guides(fill = "none")
    if (l > 1L) b1 <- b1 + guides(fill = "none")   # one guide for the whole figure
    list(b1, b2)          # returned FLAT; wrapped once below, so no nesting
  }
  contrast_fig <- wrap_plots(do.call(c, lapply(seq_len(r), mk_contrast)), ncol = 2) +
  ...
  if (!is.null(diag_df) && nrow(diag_df)) {
    ...
    mk_cal <- function(l) {
      ...
      if (l > 1L) g <- g + guides(color = "none")   # one guide for the whole figure
      g
    }
  ...
  if (nrow(dL)) {
    ...
    for (l in seq_len(r)) {
      ...
      p_lpw <- p_lpw + coord_cartesian(ylim = c(0, 1.02)) + guides(color = "none")
      p_lw <- ggplot(stLlx, aes(.xf, width_signal, color = estimator, group = estimator)) +
        ...
        scale_x_discrete(drop = FALSE, breaks = xbreaks_real(stLl$n),
                         expand = expansion(add = c(0.4, 0.2))) + th
      p_lw <- add_end_labels(p_lw, stLl, "width_signal", gap = NA) + guides(color = "none")
      p_lw <- p_lw + coord_cartesian()
      lambda_figs[[l]] <- wrap_plots(p_lcov, p_lpw, p_lw, ncol = 3) +
        plot_layout(guides = if (USE_LEGEND) "collect" else "keep") +
        plot_annotation(
